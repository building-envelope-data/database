...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260630163409_DeferDeleteChecksForEntitiesThatCanOnlyBeDeletedTogether') THEN
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260630163409_DeferDeleteChecksForEntitiesThatCanOnlyBeDeletedTogether') THEN
    DROP FUNCTION "database"."LC_TRIGGER_get_https_resource_root_can_only_be_deleted_alongsid"() CASCADE;
    END IF;
END $EF$;
...
BEGIN
    ...
    DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE PROCEDURE "database"."LC_TRIGGER_data_access_policy_can_only_be_deleted_after_data"();
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260630163409_DeferDeleteChecksForEntitiesThatCanOnlyBeDeletedTogether') THEN
    CREATE FUNCTION "database"."LC_TRIGGER_resource_root_can_only_deleted_alongside_its_data"() RETURNS trigger as $LC_TRIGGER_resource_root_can_only_deleted_alongside_its_data$
    BEGIN
    ...
    DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE PROCEDURE "database"."LC_TRIGGER_resource_root_can_only_deleted_alongside_its_data"();
    END IF;
END $EF$;
...
BEGIN
    ...
    VALUES ('20260630163409_DeferDeleteChecksForEntitiesThatCanOnlyBeDeletedTogether', '10.0.9');
    END IF;
END $EF$;
